...
extern "C" {
    ...
    struct llama_context_params {
        ...
        int32_t  n_mtp;           // MTP prediction mode: 1 = disabled, 2 = predict 1 additional token, 3 = predict 2 additional tokens, etc.
    ...
    // Get MTP (Multi-Token Prediction) logits - additional token predictions
    // Returns NULL if MTP is not available or enabled
    LLAMA_API float * llama_get_logits_mtp(struct llama_context * ctx);

    // MTP logits for the ith token, similar to llama_get_logits_ith
    // Returns NULL for invalid ids or if MTP is not available
